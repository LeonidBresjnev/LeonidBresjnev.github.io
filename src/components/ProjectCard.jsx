...
export function ProjectCard({ project, delayClass }) {
  const { name, href, repoHref, icon, blurb, frameworks } = project;
  const Icon = projectIcons[icon] ?? BookOpen;

  return (
    <article
      className={`group animate-appear relative overflow-hidden rounded-3xl border border-sky-200 bg-white p-5 shadow-glow transition hover:-translate-y-1 hover:border-sky-400 ${delayClass}`}
    >
      ...
      {href || repoHref ? (
        <div
          className={`mt-6 grid gap-2 ${href && repoHref ? "grid-cols-2" : "grid-cols-1"}`}
        >
          {href ? (
            <a
              href={href}
